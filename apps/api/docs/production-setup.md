...
This document outlines the steps required to set up Osmosys Assistant for production. Following these steps will ensure that your application is configured properly for a production environment.

## Prerequisites

Before setting up Osmosys Assistant for production, ensure you have the following prerequisites with the specified versions:
...
These prerequisites are essential for deploying and running Osmosys Assistant in a environment.
...
1. **Build the Application:** Before starting the server, build Osmosys Assistant by running:
...
1. **PM2 Configuration:** Create an ecosystem.config.js (or .ts) file to configure PM2. This file defines settings such as the application name, entry point, and other options. For example:
  ...
  module.exports = {
    apps: [
      {
        name: 'osmosys-assistant', // Name of your application
...
```bash
docker exec -it osmosys-assistant-api npm run typeorm:run-migration
```
